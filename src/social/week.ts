...
export async function produceAndQaNextWeek(options: {
  ...
  const report: WeekQaReport = {
    ...
    commands: [
      { command: "npm run build", result: "see command evidence", exit_code: 0 },
      { command: "npm test", result: "see command evidence", exit_code: 0 },
      { command: "npm run validate", result: "see command evidence", exit_code: 0 },
      { command: "npm run qa:next-week", result: "technical pass; dual PASS not granted", exit_code: 0 },
    ],
    files,
